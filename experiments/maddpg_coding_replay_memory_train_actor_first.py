...
def parse_args():
    ...
    parser.add_argument("--num_train", type=int, default=300, help="number of train")
...
if __name__=="__main__":
    ...
    with tf.Session() as session:
        ...
        if(node_id==ACTOR):
            ...
            while True:
                ...
                if done or terminal:
                    iter_step=0
                    obs_n=env.reset()
                if(train_step>=arglist.batch_size*arglist.max_episode_len):
                    train_step=-1
                    break
        ...
        while True:
            ...
            #print('Num of train %d, weights broadcast end time is' %num_train, end_master_weights)
            ...
            if(node_id==ACTOR):
                ...
                while True:
                    #print(len(episode_rewards),iter_step)
            ...
            if(node_id>ACTOR):
                ...
                comm.send(trainers[node_id-2].get_weigths(),dest=0,tag=num_train)
                end_worker_weights=time.time()
                #print('Worker node %d Num train %d start sending weights time is' %(node_id, num_train), start_worker_weights)
                #print('Worker node %d Num train %d end sending weights time is' %(node_id, num_train), end_worker_weights)
                #print('Node%d' %node_id, 'Send weights to central controller', num_train)
